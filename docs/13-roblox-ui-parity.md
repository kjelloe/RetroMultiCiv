...
| City visuals: ERA × size (run-F item 8) | browser SHIPPED (specs/city-era-looks.md, ally §5d): city look varies by ERA band composing with CITY_TIERS size. Roblox item 8 = the SAME `shared/city-era.js` contract — the 4 renderer-local band ids (ancient/classicalMedieval/industrial/modernSpace) + the engine-era map (no-tech→ancient) + the style-key table, mapped to Roblox parts/materials | render-only, fog-honest (rival-under-fog→ancient); silhouette/roofline differs per band (thatch / tiled+keep / brick+stacks / slab+dome-spire), owner color on the base ring not the body; no engine/save change |
...
**WORLD-LOOK DECISION (user, Roblox run-F item 4, 2026-07-19): DEFAULT =
ENHANCED.** Retro stays available as a toggle. This resolves the
retro-vs-enhanced pick below and UNBLOCKS the bigger-lift tier + CP1 art.

**Quick-win tier (BUILT, behind the Options "world look" toggle,
ENHANCED default per the ruling above):** per-terrain built-in materials in the ENHANCED look
...
**Bigger-lift tier (UNGATED — the pick resolved to ENHANCED):**
