...
| `--no-late-join` | late join ON | Turns OFF host-wide late joining. Default behavior (since marker-0098-era builds): a game that is BOTH listed publicly AND late-joinable shows up in Find game while running or paused; a joiner takes over an AI civ (never an abandoned human seat — those stay reserved for their token) picked as the second-strongest candidate. When the last human leaves such a game it PAUSES (zero AI/regency turns — no CPU) instead of playing on; a human joining resumes it after their first End Turn. At the `--max-games` cap, creating a new game evicts ONE paused game (earliest era first, then fewest original humans, then longest-paused) — evicted games keep their save and remain rejoinable by code; active games are never evicted. If every slot is an active game, creates reject with a "server full" message. |
