...
| SO6 | turn log (classes, filters, jump-to, fog rules) | turnlog.js | PARTIAL — log present (L toggle); classes/filters/jump-to not ported. D3 diplomacy Tier-A ADDED (#1878): the three treaty events narrated via diplomacyRow (1:1 port of shared/diplomacy-view.js diplomacyEventRow) + view-derived first-contact (scanContacts parity); gate 17 pins it. Treaty events dormant until the visibility twin (bugfixer #1884) surfaces them under fog; first-contact live today |
