...
function freshWorld(seed) {
  // 56x35 with 4 civs: room enough that seed 777 survives to turn 190
...
test('fast-forward is deterministic: same seed + age → identical state hash', () => {
  ...
  const a = fastForwardTo(RULESET, freshWorld(777), age, ['p1']);
  const b = fastForwardTo(RULESET, freshWorld(777), age, ['p1']);
...
test('the grant is the era union for EVERY player, research reset', () => {
  ...
  const r = fastForwardTo(RULESET, freshWorld(777), age, ['p1']);
