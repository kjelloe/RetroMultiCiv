...
test('luau json2lua: every scenario setup and a messy save hash equal in both languages',
  { skip: !lune && 'lune not installed (dev-only toolchain)' }, async () => {
    ...
    try {
      ...
      assert.strictEqual(files.length, 67, 'the scenarios (067 WLTKD added)');
...
test('luau ai: the golden-seed sim reaches the turn-100 checkpoint bit-exact',
  { skip: !lune && 'lune not installed (dev-only toolchain)' }, () => {
    ...
    assert.match(res.stdout, /checkpoint 100: 0xb1a5b113\n/,
...
test('luau mapgen: map-type preset worlds match the JS engine and the pins',
  { skip: !lune && 'lune not installed (dev-only toolchain)' }, async () => {
    ...
    const PINS = { // W6 slice-3 (city roles + war pair) rules.json stamp cascade
      continents: '66147acc', pangaea: '124a40e5',
      archipelago: 'a15e71ae', islands: 'b01befc6'
    };
    const players = [
...
// -> 0x93e192a8 (W6 slice-3 city roles + war pair: rules.json cityRoles/airDoctrine/
...
const FF_PARITY_PIN = 'ff-parity 0x93e192a8 turn 25 grant 22';
