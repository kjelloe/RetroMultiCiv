...
3. Definition of done, every item: `node --test test/` fully green
   (currently 1063 tests), the item's own verification steps pass, related
...
## A58 — Complete the in-game encyclopedia (wave VII item 2; "later" per user — queue after A48/A49)  [helper in progress 2026-07-17: INVENTORY (#752) + A58a catalog-text extraction (#758, committed) + A58b Civilopedia panel (#765): client/ui/pedia.js full-screen overlay, 6 data categories from the rulesets (units/buildings/wonders/advances/governments/terrain) + cross-links via catalog-text; 📖 corner button + '?' key (keymap+button prior-art cited); turnlog's 3rd techUnlocks folded onto catalog-text via wonderMark (byte-identical); test-ui/pedia.spec.js + screenshot; node surface green (4 reds = bugfixer engine window). A58c (concepts, COEXIST ruling #766) + A58d (docs/13) done in #767: client/ui/pedia-concepts.js — 11 original v1-draft concept entries (engine-verified incl. ZoC); Concepts pedia category + openTo() deep-link API; ❓ quick-tips + advice cards deep-link into the pedia (coexist, ❓ not gutted); docs/13 Tier-2 rows (portable catalog-text/pedia-concepts source). test-ui specs + screenshots green. A58 COMPLETE end-to-end (inventory→A58a→A58b→A58c→A58d). pedia-concepts.js is v1-draft for the ally's editorial pass.]
...
### A67 REVIEW TABLE — CLOSED 2026-07-17 (all #1-#9 shipped, gamesim-golden-neutral, each CI-byte-proven): A67b catapult+diplomat, A67c phalanx+musketeers+riflemen, A67d knights, A67e carrier, A67f bomber+nuclear. All pure-data on the A88b payoff (recipe + UNIT_SILHOUETTE remap + chrome; zero assets.js dispatch after A88b). Nine unit types now render distinct silhouettes vs the old generic families.

### SMALL FOLLOW-UPS (client lane, any free helper):
- ✅ 3b test-harden (done, A82a batch — poll-until-retired).
- ✅ chariot typeExtra export (done, A82a batch — tools/export-asset-recipes.js exports typeExtra; roblox can switch buildUnit).
- **render-spec typeClasses rename** (reviewer #998 advisory): specs/render-spec.json's `typeClasses` key changed SHAPE (class→[types] arrays became type→recipe map) under the SAME name during A88b — in-repo consumers updated + no roblox/ reader, but an out-of-repo reader gets a silent shape change. Rename the key to `unitSilhouette` (self-describing) in tools/render-spec.js + regenerate + the render-spec test. Small, gamesim-golden-neutral.
...
**[claimed: bugfixer 2026-07-17] [done: 2026-07-17 — EVIDENCE-FIRST close, combined with N3 in ONE golden window. Reproduced the stall (seed4 all-7-civs 1-city); measured the 3 candidate fixes: (a) escort-outranks-scouting ALONE was NEGLIGIBLE (seed4 unchanged); ROOT was broader — restoring the `guards >= 2` scout-departure floor B23b dropped (a multi-city civ met the quota with its WHOLE militia and emptied EVERY garrison at once → barbs captured cities → collapse; the threat veto is reactive, barbs spawn unseen). FIX = one-line revert `scoutDepart = scouting && exploreMode!=='greedy' && guards >= 2`; quota/veto/fast+boat pools KEPT; fix (a) dropped (identical numbers, #754). Byte-shaped luau twin. Sim-runner A/B PASS (#771): A93 floors GREEN (M2 10.5/M3 71/M4 82.5), cities median 11 in the 7-34 band. Exploration ~8% deferred to the quota-table sweep (swept knob, not the mechanism — architect #764). Unit-bloat caveat (1/10 seeds) ledgered to N4 garrison-cap (#774). Doctrine flag: the sole-guard opener was the user's doctrine but the veto can't implement his "unless a visible threat" against unseen barbs — offered as a morning 1-city-only-exception option, NOT built tonight. Architect gate PASS #774.]**

**N3 — Wave-2 #1 NAVAL PROBE [claimed: bugfixer 2026-07-17] [done: 2026-07-17 — GOLDEN WINDOW (combined with B23c above). Rulings @#747/#741. Shipped (JS + byte-shaped luau twin): navyPriorityOf (per-CIV water ratio, sea tiles within rules.aiNavyRadius=6 of cities > rules.aiNavyWaterPct=30) + bestSeaUnit (earliest-available sea unit, mirror of bestAttackerUnit, excludes attack-0 transport) + seaTech + a NAVAL-TECH BEELINE slotted AFTER the monarchy beeline (economy first) + per-city ship injection (coastal city + land core >= rules.aiNavyAfterLandUnits=3 + fleet < 1-per-coastal capped rules.aiNavyTargetCap=4), above generic buildings/wonders. Ships range as B23b boat-scouts (synergy). SCOPE FENCE (build+scout only): N3 GUARD (Option B @#741) = stepEntersSea predicate + 6 decision-layer guards so AI land units never step onto sea (no A69 auto-board leak — the incidental boarding rode land units out of play forever, no unload doctrine); engine legality untouched, N3b-relaxable; verified sim-clean t221 ships=4 aboardLandUnits=0. Sim-driver invariant now exempts aboard land units (A69, legal at the ship tile). Knobs aiNavyWaterPct/Radius/AfterLandUnits/TargetCap in rules.json. New sweep test naval-probe.test.js (7, incl the guard case, all revert-proof). COMBINED GOLDEN RE-RECORD (JS==Luau all): soak 100=0x67220be7 200=0xbf549246 300=0xe28e365f 400=0xb88d908b; natural r395/p2/0x72c846cc. luau turn-100 anchor + witness (0xdae743ab, clean, LOCAL/gitignored) regenerated. Suite 439/439 zero-skip. sim-runner sweeps the naval knobs next.]**
...
**[claimed: bugfixer 2026-07-17] [done: 2026-07-17 — IDENTITY REFACTOR (GO @#783). aiWarDoctrine reshaped to oddsGatePct/defenderGatePct integers (0→0, 1→100, 2→200); warDoctrineOf returns {massSize, oddsGatePct, defenderGatePct} with a *100 FALLBACK for external tables carrying the old integer oddsGate/defenderGate (defenderGate absent → offensive gate, else 100 floor). assaultOddsOk now `attackStrength*100 >= gatePct*defenseStrength` — DECISION-IDENTICAL at 0/100/200. engageGate→engageGatePct, B24-block D.oddsGate→D.oddsGatePct; massSize unchanged. rules.json REPLACED the old fields with pct (canonical). Byte-shaped luau twin. IDENTITY PROOF: soak 0xb88d908b / natural 0x72c846cc / luau turn-100 0x67220be7 ALL BYTE-UNCHANGED (no hash shifted = the refactor is correct); luau-twins 5/5 (rules.json checksum recomputes live, no pin edit needed); defender-march.test.js kept its OLD oddsGate/defenderGate fields and stays green — PROVING the fallback. Suite 439/439 zero-skip (identical to marker-0029). Sim-runner runs the defenderGatePct sweep {0,30,40,50,60,70,100} next; USER pins the M11 default in the morning. N4 garrison cap follows.]**
...
## A99 — Onboarding advisor round 2 = EXTEND A78's advice.js (user gate-ruled 2026-07-17 last-call; re-scoped 2026-07-17 night after the helper caught the A78 collision — the original A99 spec duplicated a shipped module)  [helper done 2026-07-17 (done-mail #750): 3 state-triggered cards + PURE exported when(state,me) predicates added to advice.js — firstContactWhen (visible non-own unit), lowTreasuryWhen (gold < cities×3 upkeep proxy, flagged for tuning), fortifyGarrisonWhen (ungarrisoned own city while enemy known); wired into A78's onChange (offer()+adviceGate reuse, no new module/toggle/dup); +3 crafted-state tests, advice 6/6; golden-neutral. Full-suite reds = bugfixer's in-flight engine window (rules.json/ai.js/ai.luau), not mine.]
...
resume gamecode, no new secret invented; it identifies which saved
game to resume) to resume that specific saved game from the CLIENT:
...
## A50 — Public-host hardening (docs/12 §3 — UN-GATED 2026-07-14; NOTE 2026-07-15: A61 sets the hardened-DEFAULT posture + static whitelist FIRST; A50's items assume it)  [helper H2 in progress 2026-07-17: ITEM 1 done (join-by-id close, server/index.js lobby-branch code gate + test) + ITEM 2 done (server/limits.js per-IP rate limits + global caps, wired into connect/create/join/chat, clock-injectable; 8 unit + 3 integration tests) — done-mail #698, my surface 34/34 (full-suite reds were the bugfixer's in-flight B21 golden window, not mine). ITEM 3 core done (done-mail #702): server/rotation.js saves-rotation (count+size budget, oldest completed/abandoned first, active never evicted) wired at boot + 60s sweep; gameOver-unlist in listGames; 8 tuning CLI flags (--max-saves/-mb, --max-conns/-per-ip, --max-games, --creates-per-hour, --joins-per-min, --chat-per-min); how-to-host.md + host-guide.html docs; 8 unit + 1 integration test. SCOPED OUT as ITEM 3b (needs a lobby.js createdAt + registry.remove() + last-activity seam): unstarted-lobby TTL + abandoned-game archive. Log rotation rides A50 item 5 (no server file-logging yet). ITEM 4 (per-conn fairness budget) is a SEPARATE later measured slice per docs/16 §2.4. Items 1+2 committed 5bc6a89; item 3 + architect-requested TWO-TIER rotation fix (#703→#704: active never / completed first / resumable last under a hard budget; 12 unit + 1 integration test, surface 31/31, full suite 420/421 = only the B23b fastforward red) awaiting gate. Item 3 committed d134e94 (tiered rotation). 3b DONE (done-mail #707): lobby.js createdAt + registry.remove(); maintenanceSweep retires unstarted lobbies (--lobby-ttl-min, default 60) + abandoned started games (--abandoned-hours, default 24; save survives = resumable by code), LAN default exempt, one injectable clock; +2 socket tests; FULL SUITE 423/423 green. A50 lifecycle/rotation scope (items 1-3 + 3b) COMPLETE. REMAINING A50: item 4 (per-conn fairness budget — measured docs/16 §2.4, separate slice), item 5 (/healthz + structured logs; log-rotation rides it), item 6 (invite-code allowlist flag). docs/16 gaps 2+3 closed; re-rank waits on item 4.]
...
## A101 — Operator resource-cap flags for the host (architect #1875, user-requested; helper, golden-neutral server/lobby validation)  [claimed: helper 2026-07-19] [done: 2026-07-19 — three host flags bounding per-game resource use, enforced at game creation (composes with the #1874 OOM log fix + A50 rotation/limits so a 2–4GB host can size itself). `--max-turns N`: clamps the effective endYear to the year reached at turn N (server/lobby.js `yearAtTurn`, exported; games end at rules.endYear per engine/score.js, so a turn cap is a YEAR cap on the variable calendar; marathon's endYear 9999 → the cap; only applied when it actually shortens). `--max-civs N`: absolute per-game civ ceiling, a SILENT clamp in create()/setSlots via `civCeiling()` — the map-size seat limit still REJECTS (mapTooSmall, accurate message) when a client asks for more than the MAP holds. `--max-size S`: clamps a client's map pick DOWN to the host ceiling (`clampSize`, SIZE_ORDER). All three thread createRegistry(deps.{maxTurns,maxCivs,maxSize}) and ALSO bound the host's own --civs/--size/rulesOverrides boot game (index.js startServer). Bad --max-size exits 1 with a friendly message. Docs: how-to-host.md flags table + "Resource caps" note + specs/server-crash-resilience.md cross-ref. Tests: test/lobby.test.js +6 (yearAtTurn, --max-civs clamp+setSlots, --max-size clamp, map-size-still-rejects, marathon endYear cap, unchanged-when-not-tightening) 15/15; test/server.test.js +1 boot-path clamp 18/18; server-lobby+protocol 17/17 (50/50 across the four files). Golden-neutral: server/lobby validation only, no engine/data-ruleset/goldens touched.] [rider done: 2026-07-20 (helper, user-approved) — warn-not-fail: unknown server args WARN + continue instead of exit(1) (no cloud-init crash-loop on flags the deployed tree lacks), plus a boot `caps:` line printing the EFFECTIVE caps (maxGames/maxSavesMb/maxTurns/maxCivs/maxSize/memSoftPct, `unset` shown) so a typo'd flag is visible. Verified: bogus + typo'd flags → WARN + server up, caps line accurate; 50/50 same four files. Bundles into the A101 changeset for landing.]

## A102 — Deploy-hardening from the first live Hetzner run (architect, self-executed 2026-07-20; golden-neutral server CLI + docs)  [done: 2026-07-20 — three fixes distilled from the first real public deploy, all outside the docs/17 connect/cmd region (server/index.js + test/server-announce.test.js locked as architect). (1) `--public-addr` SCHEME GUARD: the value is split at the LAST ':' to derive host/port, so `https://host` yielded host `"https://host"` + no port and the master answered `badAddress` on every ~60s heartbeat — a silent misconfiguration that only showed in the journal. Now rejected at boot next to the existing missing-public-addr throw, with the fix in the message (bare host:port; behind a TLS proxy use the PUBLIC 443, not the internal 8123). (2) `--help`/`-h` ADDED: the A101 warn-not-fail rider's WARN text points operators at `--help`, which did not exist — it fell through to the unknown-arg branch and BOOTED A SERVER on 8123. A `USAGE` const now covers game/public-hosting/caps flags, printed then exit 0 (soak.js A100 precedent). (3) DEPLOY ALLOWLIST + cloud-init debugging documented — see the docs delta below. Tests: test/server-announce.test.js +3 (scheme rejected across https/http/ws forms, bare host:port still boots, `--help` prints usage without booting + names four representative flags) 6/6; server+protocol+lobby 41/41 unaffected. Docs: how-to-host.md § "Deploy troubleshooting" (cloud-init-runcmd-silently-incomplete as the common root cause + the ~10s-runtime tell + the replay command, then six symptom/fix entries), docs/12 §6 announce (the no-scheme rule + the A51c test ref), docs/16 §4 (a "Deploy hygiene" block: allowlist-not-exclude-list, with the ~28MB internal-content leak from the first run as the worked example), plus the two templates (cloud-init step-0 verification block; ssh-deploy header rationale). Golden-neutral: server CLI validation + help only, no engine/data/ruleset touch.]
...
## A105 — Diplomacy is offered to civs that have never been met (user report 2026-08-02; CLOSED — client + engine gate + Luau twin all landed)  [done: 2026-08-02 — `engine/diplomacy.js:116` + `luau/diplomacy.luau:144` carry the notMet guard; header updated 2026-08-05 after engine verification (the text below predates the engine window and is kept for the record)]
...
## A106 — Graphics levels: G0 baseline legibility + G1 plumbing + G2 terrain Medium (architect, self-executed 2026-08-05; client-only, golden-neutral)  [in progress: 2026-08-05 — arc design `specs/graphics-levels.md` (playtest-driven: low/medium/high display tiers; rulings: procedural-plus assets, setup picker + GPU auto-detect, WebGL2-allowed High with degradation; STANDING RULE: every art-bearing slice presents 3 screenshot variants for user selection). G0 DONE @343851b: desert palette → warm amber (user pick of 3), plains Horse special → 12-primitive standing profile (user pick of 3; five generic quadruped shapes in PROP_SHAPES), gallery rows 20–22 terrain swatch, roblox re-mirror queued to its lane. G1 DONE: `graphics: auto|low|medium|high` option (options.js DEFAULTS + ⚙ fieldset select + resolved-tier note), pure `suggestGraphicsLevel(diag)` probe (diagnostics.js; test/graphics-level.test.js GPU-string table), setup-screen picker row writing the same options key, resolve at boot in main.js + live-switch watch → renderer.setGraphicsLevel (full scene rebuild — tileTop anchors move with mesh density). G2 terrain Medium: LEVEL_SEGS {low:2, medium:4} (low byte-identical — gallery shot cmp-equal pre/post + ?vertexcheck=1), per-terrain face buckets each carrying a procedural detail CanvasTexture (new client/renderer/three/terrain-detail.js; all marks DARKEN below the ~1.95x lighting clamp — the washed-to-white trap, measured then fixed), denser ground scatter + pebbles/reeds in props.js behind the level gate, deeper dune geometry. render-spec generator: gridSegmentsPerTile = low + ByLevel table. Verified: graphics/asset/mock-state/render-spec/browser batteries green, medium --webgl1 shot byte-identical to standard, playwright layout-contract+smoke+sentry+lobby-start-mobile green. Detail-style variant: user picked v2 balanced-weave of 3. G3 DONE 2026-08-12 (overnight): SEGS 8 + PCFSoft shadows (camera-following shadow box) + pixel ratio 3 + WebGL2 degrade-to-medium (verified byte-identical to real medium-WebGL1); found + repaired the DEAD WebGL1 verification flag (--disable-es3-gl-context no-ops upstream → --disable-webgl2 everywhere, debugging/webgl-probe.js guards it); provisional High look = H2 warm-afternoon of 3 shots (user pick pending). G4 DONE: medium seg-boost ×1.5 roster-wide; recipes-high.js — all 21 silhouette bodies authored (29-unit roster) + city house-ring ×1.3 at high; provisional style = A faithful-upscale, B/C demoed on the spearman (user direction pick pending). G5 DONE: test-ui/graphics-levels.spec.js 3/3 (picker persistence, per-tier boot + triangle budget low<medium<high with 3M ceiling, ⚙ live-switch rebuild) + renderInfo()/__gfxInfo handle + README graphics section. Arc review page debugging/g3-g4-variants.html.] [v2 LADDER (H1-H5) 2026-08-13, all landed in one overnight window @marker-0115: user re-tiered after the tier-sheet review (G3 terrain = the Medium bar; High watermark = Transport World). H1 re-tier + deer/seal/fish standing motifs (user picks). H2 High smooth terrain: buildSmoothTerrain (indexed, sharpened-bilinear vertex-color blending, sand shorelines both banks, per-vertex river/fog), TW tree kit + lone meadow trees, conforming road sub-segments + centerline dashes, double surf. H3 model-grade cities (gable+chimney+windows+doors, era rooftops, rampart walls). H4 COMPLETE: recipes-model.js all 21 silhouettes at high with per-silhouette fallback. H5 budgets on the PLAIN boot (low 9,600 tris/24 calls, medium 130,152/39, high 135,416/44 xsmall; 3M ceiling ~22x headroom; debugging/measure-tris.mjs permanent) + visual goldens re-recorded from CI actuals. Two recorded traps: ?e2e=1 boots park artificial scene states (never measure budgets there); WebGL1-vs-2 byte-parity ended with sub-pixel geometry (contract = vertexcheck + WebGL1 functioning). All 9 art picks user-confirmed.] [H6-H11 zoom/detail arc 2026-08-14 (markers 0117-0121, all picks confirmed same-day): H6 tier-split high animals + clearings + horned oxen; H7 rounded-R15 figures + H7b headwear/hair (no bald heads); H8 outer-house facades + H8b SOUTH-facing (camera lesson); H9 period helmets per helmets.png (nasal/barbute/great-helm/kettle/morion) + roof-mix A; H10 era kits (wells/timber/lamps+lit/solar+pad) pick A; H11 batch: R15+helmets PROMOTED TO MEDIUM, six wonder landmarks (visible-rim anchors — camera lesson bit twice), 3 classical banners, rich high water (waves+shimmer), playtest.md, setup onboarding arrow → graphics picker, ?gfx= link param (+spec guard). Budgets re-measured 2026-08-15: low 9,600/24, medium 135,640/67, high 149,043/63. Low byte-frozen throughout; medium webgl1 byte-identical post-promotion.] [PLAYTEST ROUND 2026-08-15 (markers 0123-0125, all user-found on real hardware): (0123) onboarding arrows now FOLLOW THE SCROLL — the setup card scrolls and below-fold buttons never got arrows on short screens (only the graphics arrow showed; reproduced 1366x728, probe kept debugging/ob-probe.mjs). (0124) terrain picks DEAD AT MEDIUM since G2 — castAt tested hit.object === terrain.mesh but medium terrain is a GROUP; every open-terrain hover/tap returned null (no hover arrows/tap-to-move); fixed with a parent walk + group-pick spec guard proven red-on-revert; LESSON: a tier that changes scene STRUCTURE must re-run every identity-test path. (0125) peaks-over-peaks at High — the faceted tiers' peak/snow prop cones floated over the smooth summits; cones gated low/medium-only + vertex-color SNOWLINE snow-caps at high; water normals throttled to every 4th tick. Numbers recorded: UHD 600 medium ~48fps / low ~58; RTX 4070 high ~50 typical / 12 dips / 593MB peak. H12b perf plan (delta rebuilds, merged city geometry, instanced bodies) parked in the spec.] [H12 + H12b 2026-08-15 (markers 0126-0127): flying-boulders fixed (offset props ground via surfaceAt, crystals sink 0.45x at high); FULL 22-wonder landmark set (12 visible-rim slots, sorted assignment); city kits + wonders PROMOTED TO MEDIUM; UHD 600-617 auto-detects low. H12b DONE: delta rebuilds (map FNV signature skips tile rebuilds; per-unit/city signatures reuse baked meshes; factionsVersion/level/reveal force paths) + per-material static baking (medium/high only — LOW stays unbaked to preserve byte-identity + CI goldens, measured delta-20 shift otherwise). Draw calls: medium 67->31, high 63->25 on the boot scene; budget spec guards calls<55. 4090 numbers: 30-60fps/950MB pre-fix. Broad lane 10/10 (play flows over the delta path).] [H13 era roads 2026-08-15 @2639c41: roads restyle with the VIEWER's era at medium/high — roadStageFor (shared/city-era.js, pure): dirt path / cobblestone (renaissance) / slim unmarked (industrial) / marked classic (automobile) / four-lane highway (plastics|space-flight); rails = twin steel lines, double-tracked + widened ties from stage 3. Fog-honest by construction (viewer's own techs; roads have no owner); viewerRoadStage rides computeMapSig so an era change costs one tile rebuild; unannotated views default stage 3. LOW byte-frozen (g0 cmp + default-frame CI golden both untouched); gallery ?roadstage/?raildemo review overrides default-off; review sheet debugging/h13-road-eras.png; roadStageFor table test. 4090 re-playtest recorded: 40-57fps post-H12b, dips gone. Roblox re-mirror of shared/city-era.js twin queued to its lane. H13b roads-into-cities 2026-08-15 @12743e2 ALL TIERS (ruled a fix): city tiles draw the road's other half toward real-road neighbours, plain-road style always (rails stop at the city edge); roadAt(x,y,real) guard + no isolated stub on cities; gallery road-into-city demo at (6,6) + ?nolabels=1; gallery visual golden re-recorded from CI run 31878952454 actuals (diff confined to the row-6/7 band; CI splash matched). Occlusion lesson in art-variants skill.]
