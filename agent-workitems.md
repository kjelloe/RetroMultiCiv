...
3. Definition of done, every item: `node --test test/` fully green
   (currently 440 tests), the item's own verification steps pass, related
...
- **3b test-harden** (helper flake #886): test/server.test.js:585 'abandoned started game retired' uses a fixed wait that races the retire sweep under 6-10× parallel-suite slowdown (got gameFull vs noSuchGame). Change to poll-until-retired with a timeout (server tests already budget ~30s/msg — matches the house pattern). First occurrence; harden-not-firefight. gamesim-golden-neutral.
- **chariot typeExtra export** (helper #832 flag, roblox-helper #868 confirmed non-urgent): asset-recipes.json has the chariotWheels recipe but nothing says chariot USES it; the Roblox AssetFactory hardcodes it (renders fine today). Add a typeExtra block to tools/export-asset-recipes.js so the JSON is self-describing → roblox switches buildUnit to read it, killing the last recipe dual-source (A88b spirit). Small.
